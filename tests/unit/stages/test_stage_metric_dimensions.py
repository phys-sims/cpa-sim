...
@pytest.mark.unit
def test_stage_metric_keys_have_explicit_suffixes() -> None:
    result = run_pipeline(
        PipelineConfig(
            stretcher=PhaseOnlyDispersionCfg(name="stretcher", gdd_fs2=500.0, tod_fs3=0.0),
            fiber=FiberCfg(name="fiber"),
            amp=SimpleGainCfg(name="amp", gain_linear=1.1),
        )
    )

    stage_prefixes = ("cpa.laser_init.", "cpa.stretcher.", "cpa.fiber.", "cpa.amp.")
    stage_keys = [key for key in result.metrics if key.startswith(stage_prefixes)]
    assert stage_keys
    ...
    for key in stage_keys:
        ...
        if leaf in _DIMENSIONLESS_LEAF_KEYS:
            continue
        if not leaf.endswith(_ALLOWED_SUFFIXES):
            invalid.append(key)
...
@pytest.mark.unit
def test_laser_free_space_and_amp_dimensional_consistency() -> None:
    gain_linear = 1.4
    result = run_pipeline(
        PipelineConfig(
            stretcher=PhaseOnlyDispersionCfg(name="stretcher", gdd_fs2=750.0, tod_fs3=0.0),
            amp=SimpleGainCfg(name="amp", gain_linear=gain_linear),
        )
    )

    laser_energy = result.metrics["cpa.laser_init.laser.energy_au"]
    stretcher_energy = result.metrics["cpa.stretcher.stretcher.energy_au"]
    amp_energy = result.metrics["cpa.amp.amp.energy_au"]

    assert stretcher_energy == pytest.approx(laser_energy)
    assert amp_energy == pytest.approx(stretcher_energy * gain_linear)
...
@pytest.mark.unit
def test_wust_fiber_energy_metrics_are_unit_explicit_and_fs_to_s_consistent(
