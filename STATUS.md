...
- Scope: Hardened docs SVG determinism by setting Matplotlib `svg.hashsalt` to a fixed value and validating stable byte-identical SVG outputs across repeated runs to prevent cache churn from randomized internal SVG element IDs.
...
- Scope: Added an optional slow+physics+gnlse regression `tests/physics/test_fiber_dispersive_wave_plots.py` that runs multi-Raman (blowwood/hollenbeck/no-Raman) dispersive-wave propagation on a larger grid, validates saved full z-trace shapes and robust blue-band emergence, and conditionally writes combined wavelength-vs-distance and delay-vs-distance PNG artifacts controlled by `CPA_SIM_ARTIFACT_DIR`.
