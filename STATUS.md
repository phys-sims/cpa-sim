...
- Scope: Updated the SPM-after-fiber-amp example to keep canonical shared stage plotting while overriding the line-window threshold policy (`cpa.plot.line.threshold_fraction=1e-2`) so the emitted SPM plots use a tighter, more readable time/spectrum window; added integration assertions covering the exported policy overrides in the example summary.
