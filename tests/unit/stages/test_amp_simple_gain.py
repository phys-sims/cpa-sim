...
from cpa_sim.models import PipelineConfig, SimpleGainCfg
from cpa_sim.pipeline import run_pipeline


@pytest.mark.unit
def test_simple_gain_scales_amp_energy_metric() -> None:
    baseline = run_pipeline(PipelineConfig(amp=SimpleGainCfg(gain_linear=1.0)))
    boosted = run_pipeline(PipelineConfig(amp=SimpleGainCfg(gain_linear=4.0)))
