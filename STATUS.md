...
- Scope: Simplified all canonical example modules to no-arg top-to-bottom runs (removed argparse surfaces), introduced shared example runtime helpers, updated docs/examples to physics-first quickstarts with advanced API usage sections, switched docs asset generation to call example APIs directly, added metrics-stage input/output overlay artifacts (`metrics_time_intensity_overlay.svg`, `metrics_spectrum_overlay.svg`) with reference-axis interpolation support, retuned WUST-compatible linear wave-breaking maps for closer visual parity, added fixture-backed wave-breaking physics regression bounds, and pinned CI optional `gnlse` installs to `2.0.0` in nightly optional workflows.
