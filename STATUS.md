...
- Scope: Refactored `gnlse_dispersive_wave_showcase` into pure case-definition builders with a strict `dispersion` preset matching WUST-FOG `examples/test_dispersion.py` values (50 fs sech2 pulse at 835 nm, 16384 samples, gamma=0, blowwood Raman, z_saves=200, exact 4-term Taylor betas), added interpolation loading from a checked-in `neff_pcf_dat01.csv` table extracted from upstream `data/neff_pcf.mat`, added CLI case selection (`--case taylor|interpolation|both`) plus combined `fiber_dispersive_wave_comparison_wust.png` output, and added unit tests covering preset contracts and parser behavior without invoking heavy propagation.
