...
- Scope: Refined Treacy validation example plot windowing to derive crop limits from the original pulse spectrum/time-width references (not stretched/dominant outputs), while still reusing shared autoscale plotting helpers so phase/group-delay/polynomial comparison curves remain readable without clipping.
